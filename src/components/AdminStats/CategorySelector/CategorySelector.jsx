...
const CategorySelector = ({ categories, selectedSlug, onSelect }) => {
      ...
      {sorted.map((category) => {
        ...
        return (
          <button
            ...
            className={`category-pill ${selectedSlug === category.slug ? "selected" : ""} ${category.slug} ${isCompact ? "compact" : ""}`}
          ...
          >
            ...
            {isCompact && (
              <>
                <div className="score small">{category.averageScore.toFixed(0)}</div>
              </>
            )}
