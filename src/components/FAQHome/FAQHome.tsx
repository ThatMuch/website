...
// This prevents all items in the list from re-rendering and executing
// sanitizeHtml when the user toggles a single item, saving unnecessary
// DOM updates and expensive computations.
...
export default function FAQHome({ title, description, questions }: Props) {
  ...
  // ⚡ Bolt Optimization: Memoized the toggle event handler with useCallback
  // to prevent re-creating the function on every render, ensuring
  // FAQItem's React.memo correctly bails out of re-renders.
  const toggleQuestion = React.useCallback((index: number) => {
    setActiveIndex((prevIndex) => (prevIndex === index ? null : index));
  }, []);

  // ⚡ Bolt Optimization: Memoized the expensive sanitizeHtml call for the main description
  // using useMemo, so it only recalculates when the description prop actually changes,
  // avoiding recalculation on every accordion toggle.
